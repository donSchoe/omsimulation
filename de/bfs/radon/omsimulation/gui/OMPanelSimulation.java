...
public class OMPanelSimulation extends JPanel implements ActionListener,
  ...
  // private JLabel lblDecimalSeparator;
  ...
  /**
   * @return the randomCampaigns
   */
  public int getRandomCampaigns() {
    return randomCampaigns;
  }

  /**
   * @param randomCampaigns
   *          the randomCampaigns to set
   */
  ...
  public void updateStatus() {
    ...
    progressBarSimulation.setString(getLogMsg());
    progressBarSimulation.setValue(getStatus());
  }
  ...
  class Simulation extends SwingWorker<Void, Void> {
     ...
     * descriptive statistics for simulations with n <= 1 million. It writes a
     * CSV file with the results of the statistics in the end. This may take a
     * while, grab a coffee.
     * 
     * @param building
     *          An building consisting of all the rooms and their values.
      ...
      if (ratioThree >= 0 && ratioFour >= 0 && ratioFive >= 0 && ratioSix >= 0) {
        ...
        if (total >= 1) {
          ...
          if (absoluteTotal <= 1000000) {
            ...
            // isDescriptive = false;
            // if (campaignLengthThree > 0) {
            // tmpUpdate("Starting summary simulation with " + absoluteThree
            // + " random variations for 3 rooms.", (int) perc);
            // Random generator = new Random();
            // int[] random = new int[absoluteThree];
            // for (int n = 0; n < absoluteThree; n++) {
            // random[n] = generator.nextInt(campaignLengthThree);
            ...
            // mod = a % 100;
            // if (mod == 0) {
            // perc = (long) (((double) x / (double) absoluteTotal) * (double)
            // 100.0);
            // tmpUpdate("Status: " + perc + "%", (int) perc);
            // }
            // if (total > 1) {
            ...
            // + "% - finished for 3 different rooms.", (int) perc);
            // if (campaignLengthFour > 0) {
            // tmpUpdate("Starting summary simulation with " + absoluteFour
            // + " random variations for 4 rooms.", (int) perc);
            // generator = new Random();
            // random = new int[absoluteFour];
            // for (int n = 0; n < absoluteFour; n++) {
            // random[n] = generator.nextInt(campaignLengthFour);
            ...
            // + "% - finished for 4 different rooms.", (int) perc);
            // if (campaignLengthFive > 0) {
            // tmpUpdate("Starting summary simulation with " + absoluteFive
            // + " random variations for 5 rooms.", (int) perc);
            // generator = new Random();
            // random = new int[absoluteFive];
            // for (int n = 0; n < absoluteFive; n++) {
            // random[n] = generator.nextInt(campaignLengthFive);
            ...
            // + "% - finished for 5 different rooms.", (int) perc);
            // if (campaignLengthSix > 0) {
            // tmpUpdate("Starting summary simulation with " + absoluteSix
            // + " random variations for 6 rooms.", (int) perc);
            // generator = new Random();
            // random = new int[absoluteSix];
            // for (int n = 0; n < absoluteSix; n++) {
            // random[n] = generator.nextInt(campaignLengthSix);
          ...
          if (isDescriptive) {
            descriptiveStatistics(x, roomAmDescriptiveStats,
                ...
                cellarMaxDescriptiveStats, csvOutput, decFormat);
            tmpUpdate(
                "Storing simulation to database. This can take a few minutes.",
                (int) perc);
            ...
            db4o.close();
            tmpUpdate("Done. Stored simulation to database.", (int) perc);
          } else {
  ...
  protected void initialize() {
    ...
    // lblDecimalSeparator = new JLabel("Decimal Separator");
    // lblDecimalSeparator.setBounds(10, 308, 133, 14);
    // lblDecimalSeparator.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 11));
    // add(lblDecimalSeparator);

    // rdbtnDot = new JRadioButton("Dot (.)");
    // rdbtnDot.setSelected(true);
    // rdbtnDot.addChangeListener(new ChangeListener() {
    ...
    // }
    // });
    // rdbtnDot.setBounds(149, 303, 107, 23);
    // rdbtnDot.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 11));
    // add(rdbtnDot);
    //
    // rdbtnComma = new JRadioButton("Comma (,)");
    // rdbtnComma.addChangeListener(new ChangeListener() {
    // public void stateChanged(ChangeEvent e) {
    // if (rdbtnComma.isSelected()) {
    // rdbtnDot.setSelected(false);
    // setComma(true);
    // } else {
    // rdbtnDot.setSelected(true);
    // setComma(false);
    ...
    // }
    // });
    // rdbtnComma.setBounds(149, 329, 243, 23);
    // rdbtnComma.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 11));
    // add(rdbtnComma);
    ...
    btnRefresh.addPropertyChangeListener(this);
    ...
    comboBoxSelectProject
        .addPropertyChangeListener(new PropertyChangeListener() {
          public void propertyChange(PropertyChangeEvent arg0) {
            ...
            // rdbtnComma.setEnabled(b);
            // rdbtnDot.setEnabled(b);
            rdbtnRandom.setEnabled(b);
            rdbtnSystematic.setEnabled(b);
            // lblDecimalSeparator.setEnabled(b);
    ...
    spnrRatio3.setEnabled(false);
    spnrRandomCampaigns.setEnabled(false);
    // rdbtnComma.setEnabled(false);
    // rdbtnDot.setEnabled(false);
    rdbtnRandom.setEnabled(false);
    rdbtnSystematic.setEnabled(false);
    // lblDecimalSeparator.setEnabled(false);
